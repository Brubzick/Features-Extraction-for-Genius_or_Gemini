    ...
    for ins in block:
        ...
        if inst[0] in movs:
            ...
            for stack in x86_64_stack:
                index = opPart.find('['+stack)
                if index > -1:
                    break
            if index > -1 : 
                if opPart[-1] != ']' and ',' in opPart: # 从参数寄存器中读取参数入栈
                    ...
                    reg = opPart[i1+1:]
                    f = opPart[:i1]
                    if reg not in x86_64_regs: # 非参数寄存器
                        return argsNum
                    if reg in paras: # 重复
                        return argsNum
                    paras.append(reg)
                ...
                elif opPart[-1] == ']': # 从栈中读取参数
                    if stacking:
                        firstF = fList.copy() # 从参数寄存器入栈的参数，如果参数较多，这里不包含全部参数。
                        stacking = False
                    ftmp = opPart
                    if ftmp in fList:
                        if ftmp in firstF: # 从栈中读取参数
                            firstF.remove(ftmp)
                        else: # 重复读取
                            return argsNum
                    else:
